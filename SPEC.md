...
| D24 | Exam calendar filter | The school calendar is filtered to **the subjects in the student's timetable** by default; alternatives are a hand-picked subject list and "all tests". With no timetable nothing is hidden. | Decided | Product owner request: show only the tracks that are mine. | F0 |
